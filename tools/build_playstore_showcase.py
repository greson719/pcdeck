"""
PCDeck — Final Master Google Play Store Screenshot & Graphic Generator
Fixes alpha composite masking, guarantees 100% visible, vibrant phone screen displays,
and renders ultra-high-converting 1920x1080 (16:9) showcase slides + 1024x500 Feature Graphic.
"""
...
def create_phone_mockup_clean(screen_img, is_horizontal=False, target_size=None, glow_accent=(0, 240, 255)):
    """
    Renders a smartphone frame with guaranteed 100% visible, sharp screenshot display.
    Properly handles alpha compositing so screen pixels are never erased.
    """
    ...
    # 5. Build Drop Shadow & Ambient Neon Halo
    pad = 45
    ...
    # Paste frame onto canvas using frame's own alpha mask
    canvas.paste(frame, (pad, pad), frame)
    return canvas


def create_pc_window_mockup_clean(window_img, target_width=720, glow_accent=(0, 240, 255)):
...
def generate_all():
    ...
    img_trackpad_v = Image.open(os.path.join(transfers_dir, "Screenshot_20260828-214527.png"))  # Trackpad
    img_keyboard_h = Image.open(os.path.join(transfers_dir, "Screenshot_20260828-214449.png"))  # Landscape Keyboard & Numpad
    img_screen_h   = Image.open(os.path.join(transfers_dir, "Screenshot_20260828-214630.png"))  # 60FPS Screen Mirroring
    img_files_v    = Image.open(os.path.join(transfers_dir, "Screenshot_20260828-214521.png"))  # File Manager
    img_audio_v    = Image.open(os.path.join(transfers_dir, "Screenshot_20260828-214532.png"))  # 48kHz Stereo Audio & Media
    ...
    pc_mock = create_pc_window_mockup_clean(pc_window, target_width=640, glow_accent=CYAN_ACCENT)
    phone_hero = create_phone_mockup_clean(img_trackpad_v, is_horizontal=False, target_size=680, glow_accent=CYAN_ACCENT)
    ...
    phone_trackpad = create_phone_mockup_clean(img_trackpad_v, is_horizontal=False, target_size=820, glow_accent=CYAN_ACCENT)
    render_master_slide(
    ...
    phone_kbd = create_phone_mockup_clean(img_keyboard_h, is_horizontal=True, target_size=980, glow_accent=PURPLE_ACCENT)
    render_master_slide(
    ...
    phone_screen = create_phone_mockup_clean(img_screen_h, is_horizontal=True, target_size=980, glow_accent=LIME_ACCENT)
    render_master_slide(
    ...
    phone_files = create_phone_mockup_clean(img_files_v, is_horizontal=False, target_size=820, glow_accent=CYAN_ACCENT)
    render_master_slide(
    ...
    phone_audio = create_phone_mockup_clean(img_audio_v, is_horizontal=False, target_size=820, glow_accent=YELLOW_ACCENT)
    render_master_slide(
    ...
    phone_qr = create_phone_mockup_clean(img_qr_v, is_horizontal=False, target_size=820, glow_accent=CYAN_ACCENT)
    render_master_slide(
    ...
    generate_feature_graphic_clean(pc_window, img_trackpad_v)
...
def generate_feature_graphic_clean(pc_window, img_trackpad):
    ...
    # Mockups
    pc_mock = create_pc_window_mockup_clean(pc_window, target_width=370, glow_accent=CYAN_ACCENT)
    phone_mock = create_phone_mockup_clean(img_trackpad, is_horizontal=False, target_size=420, glow_accent=CYAN_ACCENT)
